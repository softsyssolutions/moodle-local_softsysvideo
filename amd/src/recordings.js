...
define(['core/ajax', 'core/notification'], function(Ajax, Notification) {
    ...
    function renderPagination(page, totalPages) {
        ...
        if (prevBtn && page > 1) {
            prevBtn.addEventListener('click', function() {
                loadRecordings(page - 1, currentSearch);
            });
        }
    ...
    function buildFilterParams() {
        ...
        var stateEl = document.getElementById('ssv-filter-state');
        if (stateEl && stateEl.value && stateEl.value !== 'all') {
            params += '&state=' + encodeURIComponent(stateEl.value);
        } else if (stateEl && stateEl.value === 'all') {
            params += '&state=all';
        }
    ...
    function loadRecordings(page, search) {
        currentPage = page;
    ...
    function initFilters() {
        ...
        if (applyBtn) {
            applyBtn.addEventListener('click', function() {
                loadRecordings(1, currentSearch);
            });
        }
    }
    ...
    return {
        /**
         * Initialises the recordings module.
         *
