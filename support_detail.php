...
$jsstrings = [
    ...
    'system_author' => get_string('system_author', 'local_softsysvideo'),
];
...
echo html_writer::div(
    html_writer::tag('h2', get_string('ticket_detail', 'local_softsysvideo'), ['class' => 'mb-0']) .
    html_writer::link(
        new moodle_url('/local/softsysvideo/support.php'),
        '&laquo; ' . get_string('back_to_list', 'local_softsysvideo'),
...
$descrow = html_writer::div(
    '',
    'mt-3 p-3 rounded d-none',
    ['id' => 'ssv-detail-description', 'style' => 'background:var(--ssv-bg);border:1px solid var(--ssv-border);']
);
echo html_writer::div(
    html_writer::div(
    ...
    'ssv-connected-card card mb-4 d-none',
    ['id' => 'ssv-detail-card']
);

// Messages section (hidden until data loads).
$messagesheading = html_writer::tag('h5', get_string('messages_title', 'local_softsysvideo'), ['class' => 'mb-3 fw-bold']);
